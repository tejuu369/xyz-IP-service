        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-left">
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            <button
              ...
              className="px-7 py-3.5 rounded-xl bg-gold-500 hover:bg-gold-600 text-charcoal-950 font-bold text-xs uppercase tracking-[0.15em] shadow-lg shadow-gold-500/20 transition-all flex items-center space-x-2"
            >
            ...
            <a
              ...
              className="px-6 py-3.5 rounded-xl bg-charcoal-900 hover:bg-charcoal-800 text-charcoal-300 border border-white/10 text-xs font-semibold tracking-wide transition-colors"
            >
              View Deliverables
            </a>
          </div>
      ...
      <section className="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-left">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-7 space-y-8">
            <div>
              <span className="font-mono text-xs text-gold-400 font-bold uppercase tracking-wider block mb-2">
                TECHNICAL RIGOR & METHODOLOGY
              </span>
              <h2 className="text-2xl sm:text-3xl font-serif font-bold text-white">
                Practice Overview
              </h2>
              <div className="text-charcoal-300 text-sm sm:text-base leading-relaxed mt-4 whitespace-pre-line space-y-4">
                {pageData.overview}
              </div>
            </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {pageData.keyAspects.map((aspect, idx) => (
                    <div
                      key={idx}
                      className="p-3.5 rounded-xl bg-charcoal-900 border border-white/10 flex items-start space-x-2.5 text-xs text-charcoal-200"
                    >
              ...
              <div>
                ...
                <div className="space-y-4">
                  {pageData.workflow.map((w, idx) => (
                    <div
                      key={idx}
                      className="p-5 rounded-2xl bg-charcoal-900/60 border border-white/10 flex flex-col sm:flex-row sm:items-center gap-4"
                    >
                      <div className="font-mono text-sm font-bold text-gold-400 px-3 py-1.5 rounded-lg bg-gold-500/10 border border-gold-500/30 w-fit flex-shrink-0">
                        {w.step}
                      </div>
                      <div>
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            {pageData.deliverables && pageData.deliverables.length > 0 && (
              <div id="service-deliverables" className="p-6 rounded-3xl bg-charcoal-900 border border-white/10 space-y-4">
                <div className="flex items-center space-x-2 text-xs font-mono text-gold-400 font-bold uppercase">
                ...
                <div className="space-y-2 pt-2 border-t border-white/10">
                  {pageData.deliverables.map((item, idx) => (
                    <div key={idx} className="flex items-start space-x-2 text-xs text-charcoal-300">
                ...
                <div className="pt-3 border-t border-white/10">
                  <button
                    onClick={() => openConsultationModal(pageData.title)}
                    className="w-full py-3 px-4 rounded-xl bg-gold-500 hover:bg-gold-600 text-charcoal-950 font-bold text-xs uppercase tracking-wider transition-colors shadow-lg shadow-gold-500/20"
                  >
            ...
            <div className="p-6 rounded-2xl bg-charcoal-950 border border-white/10 text-xs text-charcoal-400 space-y-2">
            ...
            {relatedServices.length > 0 && (
              <div className="p-6 rounded-2xl bg-charcoal-900/40 border border-white/10 space-y-3">
                <h4 className="text-xs font-mono font-bold uppercase tracking-wider text-charcoal-400">
                ...
                <div className="space-y-2">
                  {relatedServices.map((rel, i) => (
                    <button
                      ...
                      className="w-full p-2.5 rounded-xl bg-charcoal-950 hover:bg-charcoal-900 border border-white/10 text-left text-xs font-semibold text-charcoal-300 hover:text-white flex items-center justify-between transition-colors group"
                    >
                      <span className="truncate">{rel.title}</span>
                      <ArrowRight className="w-3.5 h-3.5 text-charcoal-500 group-hover:text-gold-400 transition-colors flex-shrink-0" />
                    </button>
                  ))}
                </div>
              </div>
