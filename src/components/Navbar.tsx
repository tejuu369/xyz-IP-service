      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            {/* Desktop Consultation CTA & Admin Portal in svpconstructions style */}
            <div className="hidden lg:flex items-center space-x-2.5">
              <button
                id="admin-portal-nav-btn"
                onClick={() => navigate('/admin')}
                className={`relative inline-flex items-center justify-center px-3.5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-[0.12em] transition-all duration-200 border ${
                  currentPath === '/admin'
                    ? 'bg-gold-500 text-charcoal-950 border-gold-500 shadow-md shadow-gold-500/20'
                    : 'border-white/10 text-charcoal-300 hover:text-gold-400 hover:border-gold-500/40 bg-charcoal-900/60'
                }`}
                title="Admin Portal - View Enquiries"
              >
                <ShieldAlert className="w-3.5 h-3.5 mr-1.5 text-gold-500" />
            ...
            <div className="flex lg:hidden items-center space-x-2">
              <button
                onClick={() => navigate('/admin')}
                className="px-2.5 py-1.5 rounded-lg border border-gold-500/30 text-gold-400 bg-gold-500/10 text-xs font-bold uppercase tracking-wider flex items-center space-x-1"
                title="Admin Portal"
              >
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden border-b border-white/10 bg-charcoal-950 max-h-[85vh] overflow-y-auto px-4 py-6 space-y-4 animate-in slide-in-from-top-4 duration-200">
            {/* Admin Portal Quick Access Card */}
            <div className="p-3.5 rounded-xl bg-gold-500/10 border border-gold-500/30 flex items-center justify-between">
              <div>
                <div className="flex items-center space-x-2 text-xs font-bold text-gold-400 uppercase tracking-wider">
                  <ShieldAlert className="w-4 h-4 text-gold-500" />
                  <span>Admin Portal</span>
                </div>
                <div className="text-[11px] text-charcoal-400 mt-0.5">
                  View & manage client enquiries
                </div>
              </div>
              <button
                onClick={() => {
                ...
                className="px-3 py-1.5 rounded-lg bg-gold-500 hover:bg-gold-600 text-charcoal-950 text-xs font-bold uppercase tracking-wider transition-colors shadow-sm"
              >
                Access
              </button>
            </div>
